...
class ThreadCommandType extends CommandType {
  ...
  async execute(args) {
    ...
    // canSpawnThreads marks a human-steered thread whose LLM may itself use
    // create_thread. A user-created /thread sets it up front; other threads gain
    // it only when a human sends a message into them (promoteThreadSpawnCapable in
    // the worker). Threads with neither are leaf workers and the worker withholds
