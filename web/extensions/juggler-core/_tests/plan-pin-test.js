...
export async function runTests(_ctx) {
  ...
  await test('no plan anywhere says so, briefly', () => {
    const m = mount(null);
    assert((m.body.textContent || '').trim() === 'No plan.',
      `expected 'No plan.', got ${JSON.stringify(m.body.textContent)}`);
    m.teardown();
  });
  ...
  await test('a plan appearing where there was none replaces the empty state', () => {
    const m = mount(null);
    assert((m.body.textContent || '').trim() === 'No plan.', 'expected the empty state first');
