...
| `delegated` | boolean | Marks a thread spawned by a `delegatesToSubthread` tool rather than by `create_thread` or a human. Read server-side by `withinDelegatedThread` — which disables delegating tools anywhere below it, so sub-agents cannot recurse — and client-side by `MessageThread.isDelegated`, which hides that column's strategy selector: the thread's strategy belongs to the tool that called it and is not the user's to change. |
