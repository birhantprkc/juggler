...
// default` is always emitted — without it the CLI resolves its permission mode
// from settings files shared with the user's interactive sessions in the same
// folder, so a persisted plan mode would block every tool.
	...
	for i, a := range args {
		if a == "--permission-mode" {
			if i+1 >= len(args) || args[i+1] != "default" {
				t.Fatalf("--permission-mode must be followed by %q; got %v", "default", args)
			}
			return
		}
	}
