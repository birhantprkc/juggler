...
// LEADING context-item messages, before history (the only placement now). The
// rolling breakpoint must land on the last stable HISTORY block, so a leading
// context item sits inside the cached prefix (paid once) rather than anchoring
