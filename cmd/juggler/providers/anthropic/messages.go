...
func transformToAPIMessagesInternal(messages []provider.Message) []APIMessage {
	...
	for _, msg := range messages {
		...
		case "thinking":
			...
			// back and rejects a signatureless one (400). The worker does not
			// persist thinking signatures today, and a thinking block generated by
			// a different provider (e.g. DeepSeek, whose reasoning carries no
			// Anthropic signature) can reach here after a mid-conversation provider
			// switch — so drop any thinking block without a signature rather than
			// 400 the turn. This preserves the prior behavior (thinking was dropped
			// entirely before buildMessages emitted it) while still round-tripping
