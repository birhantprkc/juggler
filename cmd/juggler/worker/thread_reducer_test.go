...
// TestUpcomingTurnDeliversToolResults pins the predicate that gates the
// strategy loop's queued-message promote: it must report true ONLY when the
// next turn is a fully-completed, non-cancelled tool-result continuation — the
// model finishing the response it paused to call tools, which a freshly-typed
// user message must not be spliced into.
func TestUpcomingTurnDeliversToolResults(t *testing.T) {
	cases := []struct {
		name  string
		items []ConversationItem
		want  bool
		...
		{
			name:  "trailing user message (fresh turn, not a tool continuation)",
			items: []ConversationItem{userMsg("hi")},
			want:  false,
		},
		{
			name:  "trailing assistant text (turn already ended)",
			items: []ConversationItem{userMsg("hi"), assistantMsg("done")},
			want:  false,
		},
		{
			name: "single completed tool batch",
			items: []ConversationItem{
				userMsg("run ls"), assistantMsg("running"),
				toolAction("call_1", StateCompleted),
			},
			want: true,
		},
		{
			name: "multi-tool batch, all completed",
			items: []ConversationItem{
				userMsg("run two"), assistantMsg("running"),
				toolAction("call_1", StateCompleted),
				toolAction("call_2", StateCompleted),
			},
			want: true,
		...
		{
			name: "batch with one still running (not all terminal)",
			items: []ConversationItem{
				userMsg("run two"), assistantMsg("running"),
				toolAction("call_1", StateCompleted),
				toolAction("call_2", StateRunning),
			},
			want: false,
		},
		{
			name: "pending tool awaiting approval",
			items: []ConversationItem{
				userMsg("run ls"), assistantMsg("running"),
				toolAction("call_1", StatePending),
			},
		...
		{
			name: "batch with a cancelled (denied) tool — cannot continue warm",
			items: []ConversationItem{
				userMsg("run two"), assistantMsg("running"),
				toolAction("call_1", StateCompleted),
				toolAction("call_2", StateCancelled),
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := upcomingTurnDeliversToolResults(tc.items); got != tc.want {
				t.Errorf("upcomingTurnDeliversToolResults = %v, want %v", got, tc.want)
			}
		})
	}
}
