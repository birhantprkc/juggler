...
// TestStuckToolLivenessSweep_ProbesAfterGrace verifies the tool-liveness backstop
// (sweepStuckRunningTools): a tool-action stuck in running-with-no-result is not
// probed within the grace window, is probed once past grace, and is not re-probed
// within the rate-limit interval — while a running tool WITH a result and an
// approved tool are never probed.
func TestStuckToolLivenessSweep_ProbesAfterGrace(t *testing.T) {
	h := newReattachHarness(t, "conv-liveness-sweep")
	w := h.w

	// Replace the engine callback with one that also captures probe-tool-liveness
	// ids (still answering the flush sentinel so h.flush stays a barrier).
	probesCh := make(chan string, 64)
	w.SetCallback("engine", func(b []byte) {
		...
		case "probe-tool-liveness":
			probesCh <- m.ToolUseID
		}
	})
	drainProbes := func() map[string]int {
		out := map[string]int{}
		for {
			select {
			case id := <-probesCh:
	...
	// tu-stuck: the wedge shape (running, no result). tu-live: running WITH a
	// result (already producing). tu-approved: a fresh re-run. Only tu-stuck may
	// ever be probed.
	w.doc.InsertMessage(0, ConversationItem{Type: ItemTypeToolAction, ItemID: "ta-s", ToolUseID: "tu-stuck", ToolName: "bash", State: StateRunning})
	w.doc.InsertMessage(1, ConversationItem{Type: ItemTypeToolAction, ItemID: "ta-l", ToolUseID: "tu-live", ToolName: "bash", State: StateRunning, Result: resultDone})
	w.doc.InsertMessage(2, ConversationItem{Type: ItemTypeToolAction, ItemID: "ta-a", ToolUseID: "tu-approved", ToolName: "bash", State: StateApproved})

	// First sweep: starts the grace clock, probes nothing.
	w.sweepStuckRunningTools()
	h.flush(t)
	if got := drainProbes(); len(got) != 0 {
		t.Fatalf("first sweep (within grace) must not probe, got %v", got)
	}
	if _, ok := w.runningToolFirstSeenMs["tu-stuck"]; !ok {
		t.Fatal("first sweep should record tu-stuck's first-seen time")
	}

	// Backdate first-seen past the grace window, then sweep: tu-stuck is probed once.
	w.runningToolFirstSeenMs["tu-stuck"] = time.Now().UnixMilli() - livenessProbeGraceMs - 1000
	w.sweepStuckRunningTools()
	h.flush(t)
	got := drainProbes()
	if got["tu-stuck"] != 1 {
		t.Fatalf("past-grace sweep: want exactly 1 probe for tu-stuck, got %d (all: %v)", got["tu-stuck"], got)
	}
	if got["tu-live"] != 0 || got["tu-approved"] != 0 {
		t.Fatalf("only the running-with-no-result tool may be probed, got %v", got)
	}

	// Immediate re-sweep: rate-limited, no new probe.
	w.sweepStuckRunningTools()
	h.flush(t)
	if got := drainProbes(); len(got) != 0 {
		t.Fatalf("re-sweep within the probe interval must not re-probe, got %v", got)
	}

	// Once the tool leaves the wedge shape (a result lands), its bookkeeping is pruned.
	w.doc.UpdateToolActionFieldsRecursive("tu-stuck", map[string]any{"state": StateCompleted, "result": map[string]any{"content": "late"}})
	w.sweepStuckRunningTools()
	h.flush(t)
	if _, ok := w.runningToolFirstSeenMs["tu-stuck"]; ok {
		t.Fatal("completed tool's first-seen bookkeeping should be pruned")
	}
}
